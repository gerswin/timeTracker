...
- [x] Cola: `attempts` se incrementa solo en rechazos de payload (400/413/422); outages de red/auth reintentan sin límite de attempts (el tope offline es la edad, 14 días) (índice añadido); `gc()` por edad/attempts/filas (`QUEUE_MAX_AGE_DAYS`=14, `QUEUE_MAX_ATTEMPTS`=50, `QUEUE_MAX_ROWS`=100000) corre en el loop de heartbeat; filas indescifrables se ponen en cuarentena (nunca se borran ante error de descifrado) con sonda de la fila más nueva para distinguir poison localizado de clave global incorrecta (ver Limitaciones conocidas, resuelto 2026-07-14)
...
- [x] **Head-of-line en fallo sistémico de descifrado** (resuelto 2026-07-14): filas indescifrables (p.ej. `state.json` regenerado → nuevo `device_id` → AAD viejo) ya no bloquean las buenas de atrás. `fetch_batch_decrypted` sondea la fila más nueva: si la identidad está sana, pone el prefijo poison en cuarentena (`quarantined=1`, columna nueva + migración) para que las buenas fluyan; si la clave es global-incorrecta (ni la más nueva descifra), no toca nada y bloquea, esperando reinicio con la clave correcta. **Nunca borra ante error de descifrado** (borrar arriesgaba pérdida de datos en fallo transitorio); la GC por edad limpia los quarantined a 14 días. Reemplaza el viejo `SYSTEMIC_DECRYPT_MIN_BATCH`/borrado. Cubierto por 9 tests en `queue.rs` (cuarentena, avance de prefijo, bloqueo+recuperación con clave correcta, migración de esquema legacy).
